...
class MyDataset(Dataset):
    def __init__(self, input_file,word_idx, batchsize= opt.batch_size, max_doc_len = 75, max_sen_len = 45,test=False,transforms=None):
        ...
        self.y_position, self.y_cause, self.x, self.sen_len, self.doc_len = map(np.array, [self.y_position, self.y_cause, self.x, self.sen_len, self.doc_len])
...
class biLSTM(torch.nn.Module):
    def __init__(self, word_embedding, keep_prob1, keep_prob2,input_size=opt.n_hidden*2,hidden_size=opt.n_hidden):
        ...
        self.causebilstm = nn.LSTM(input_size = input_size+2, hidden_size=hidden_size,bidirectional=True,batch_first=True)
    ...
    def forward(self, x,sen_len,doc_len,keep_prob1,keep_prob2 ):
        ...
        pred_pos = F.softmax(self.nnlayer_pos(s1))
        pred_pos = torch.reshape(pred_pos, [-1,opt.max_doc_len, opt.n_class])


        ss = self.attention(inputs,sen_len)
        ss = torch.cat([ss,pred_pos],2)
        r_out, (h_n, h_c) = self.causebilstm(ss)
        ss = r_out
        ss1 = torch.reshape(ss, [-1, 2 * opt.n_hidden])
        ss1 = torch.nn.Dropout( 1.0-self.keep_prob2)(ss1)
        pred_cause = F.softmax(self.nnlayer_cause(ss1))
        pred_cause = torch.reshape(pred_cause, [-1, opt.max_doc_len, opt.n_class])


        reg = torch.norm(self.nnlayer_cause.weight) + torch.norm(self.nnlayer_cause.bias)
        reg = reg+(torch.norm(self.nnlayer_pos.weight) + torch.norm(self.nnlayer_pos.bias))
        reg = reg / 2
...
def run():
    ...
    for fold in range(1,11):
        ...
        train_staticDataset=MyDataset(train,word_id_mapping,batchsize=opt.batch_size,max_sen_len=opt.max_sen_len,max_doc_len=opt.max_doc_len,test=True)
        ...
        for i in range(opt.training_iter):
            ...
            with torch.no_grad():
                ...
                acc, p, r, f1 = acc_prf(alltestpred_cause_op.numpy(), alltesty_cause_op.numpy(), alldoc_len.int().numpy())
        ...
        get_pair_data(save_dir + test_file_name, NLP_Dataset['test'].doc_id, NLP_Dataset['test'].doc_len, NLP_Dataset['test'].y_pairs, te_pred_y_cause.numpy(),
                      te_pred_y_pos.numpy(), NLP_Dataset['test'].x, NLP_Dataset['test'].sen_len, word_idx_rev)
        get_pair_data(save_dir + train_file_name, train_staticDataset.doc_id, train_staticDataset.doc_len, train_staticDataset.y_pairs, tr_pred_y_cause,
